    ...
    def connect(self):
        try:
            self.connection = psycopg.connect(
                f"postgresql://{self.DATABASE_HOST}/{self.DATABASE_NAME}", #1                 f"postgresql://{self.DATABASE_HOST}/{self.DATABASE_NAME}",row_factory=dict_row)                           postgresql://postgres:password@book_store_db/book_store      #2 postgresql://localhost/bookstore_flask
